...
class GetGoogleCredentials:
    ...
    def get_google_auth_url(self, config, forward_url):
        try:
            self.google_flow = InstalledAppFlow.from_client_config(client_config = config, scopes = ["https://www.googleapis.com/auth/assistant-sdk-prototype"], redirect_uri = forward_url)
        except:
    ...
    def get_google_credentials(self, auth_response):
        try:
            self.google_flow.fetch_token(authorization_response = auth_response)
        except:
            return {'authorized': False}
        else:
            return {'authorized': True ,'credentials': self.google_flow.credentials}